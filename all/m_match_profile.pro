...
;
; CALLING SEQUENCE:
;   m_match_profile , datainfo, astrohi, astroh2    
; INPUTS:
...
pro m_match_profile,  datainfo, astrohi, astroh2
       if  N_params() LT 1  then begin 
          print, 'Syntax - ' +$
         'm_match_profile,  datainfo, astrohi, astroh2 (v1.1)'
       return
       endif
 close, /all
...
;read semimajor/minor axis
 ...
 fits = MRDFITS(COimage[i], 0, fitsheadHI, /dscale, /silent)
  
  
  
  ;display image
  IF(N_ELEMENTS(fits[*,0]) LT 800) THEN RESCALE=1
  IF(N_ELEMENTS(fits[*,0]) GE 800 AND N_ELEMENTS(fits[*,0]) LT 1200) THEN RESCALE=2
  IF(N_ELEMENTS(fits[*,0]) GE 1200) THEN RESCALE=3
...
;extract HI profile
  ...
  HIxcen=(Xcenter[i]-HICRVAL1[i])/HICDELT1[i]+HICRPIX1[i]    
  HIycen=(Ycenter[i]-HICRVAL2[i])/HICDELT2[i]+HICRPIX2[i]    
  ...
  COxcen=(Xcenter[i]-COCRVAL1[i])/COCDELT1[i]+COCRPIX1[i]    
  COycen=(Ycenter[i]-COCRVAL2[i])/COCDELT2[i]+COCRPIX2[i]    
