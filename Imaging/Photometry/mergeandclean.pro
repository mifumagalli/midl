...
;just on size. This can be refined later with 
;color informations
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
for ff=0, nfilt-1 do begin
    ...
    ;;Smaller than psf 
    ;;Highly elongated  
    ;;Flaged as defects by sextractor (4 and above: saturated, edge,
    ;;incomplete or currupted, overflow)
    ...
    sexdef=where(obj.FLAGS[ff] ge 4 and obj.detfilt[ff] gt 0, nsex)  
...
;;copy over the id in all the fileds
ref_str.(0)[nloop]=ref_str.(0)[0]

;;copy all the remaining tags
...
str_new=replicate(create_struct('DISTMATCH', [0.0,replicate(0.0,nele-1)]),nline)
obj_str=struct_addtags(obj_str,str_new)
...
;;my tags
;;eazy photo-z
str_new=replicate(create_struct('PHOTOZ', replicate(0.0,nele)),nline)
obj_str=struct_addtags(obj_str,str_new)
;;color flux
...
obj_str=struct_addtags(obj_str,str_new)
;;calibrated color mag, aper corr
str_new=replicate(create_struct('COLOR_MAGCAL_COR', replicate(0.0,nele)),nline)
obj_str=struct_addtags(obj_str,str_new)
;;calibrated color mag error aper corr
str_new=replicate(create_struct('ERRCOLOR_MAGCAL_COR', replicate(0.0,nele)),nline)
obj_str=struct_addtags(obj_str,str_new)
;;total SN
str_new=replicate(create_struct('TOT_SN', replicate(0.0,nele)),nline)
obj_str=struct_addtags(obj_str,str_new)
;;color SN
str_new=replicate(create_struct('COLOR_SN', replicate(0.0,nele)),nline)
obj_str=struct_addtags(obj_str,str_new)
end
...
if(instr eq 'LRIS' or instr eq 'LBC' ) then radmat=0.3 ;;match in 0.3" arcsec for ground based
