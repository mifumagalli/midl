...
;  with the extra photometry field
;
;
;-


pro calibrate_ge_gr, list, airmass, path=path, extinc=extinc, residual=residual, calib=calib, err_calib=err_calib, $
                    save=save, limmag=limmag, outresidual=outresidual


;;calibrate extra with extra-R color
...
for i=0, n_elements(list)-1 do begin
    ...
    good=where(fits.MATCHM_EXT ne 0 and fits.MATCHM_R ne 0 and fits.MATCHM_EXT-fits.MATCHM_R lt 50 and $
    ...
    if(i eq 0) then begin
        ...
        instr_ext=fits[good].MATCHM_EXT
        instr_err_ext=fits[good].ERR_MATCHM_EXT

        instr_extr=fits[good].MATCHM_EXT-fits[good].MATCHM_R
        instr_err_extr=sqrt(fits[good].ERR_MATCHM_EXT^2+fits[good].ERR_MATCHM_R^2.)
        ...
        instr_ext=[instr_ext,fits[good].MATCHM_EXT]
        instr_err_ext=[instr_err_ext,fits[good].ERR_MATCHM_EXT]

        instr_extr=[instr_extr,fits[good].MATCHM_EXT-fits[good].MATCHM_R]
        instr_err_extr=[instr_err_extr,sqrt(fits[good].ERR_MATCHM_EXT^2+fits[good].ERR_MATCHM_R^2.)]
...
;;do the calibration 
xarray=instr_extr
err_xarray=instr_err_extr
yarray=sloan_g-instr_ext-airmass_term-residual_term
err_yarray=sqrt(sloan_err_g^2+instr_err_ext^2)
...
for it=0, 5 do begin
    ...
    ploterror, new_xarray, new_yarray, new_err_xarray, new_err_yarray, psym=1, xtitle='E-R', $
            ytitle=Textoidl('g-E_{ins}-AM'), title='Iter '+rstring(it)
...
;;replot on original data
    ...
    ploterror, xarray, yarray, err_xarray, err_yarray, psym=1, xtitle='E-R', $
         ytitle=Textoidl('g-E_{ins}-AM'), title='Final '+"ZP "+string(calib[0],format="(F6.3)")+$
...
for i=0, n_elements(list)-1 do begin
    ...
    fits=mrdfits(path+list[i],1,/sil)
    fits.GMAG=asinh2pogs(fits.GMAG,"g")
    good=where(fits.MATCHM_EXT ne 0 and fits.MATCHM_R ne 0 and $
               fits.MATCHM_EXT-fits.MATCHM_R lt 50 and fits.GMAG lt limmag, ngood)
    ...
    extcal=instr_ext+calib[0]+calib[1]*(instr_extr)+airmass_term+residual[i]
    extcal_err=sqrt(instr_err_ext^2+err_calib[0]^2+instr_err_extr^2)
    res=sloan_g-extcal
    res_err=(extcal_err^2+sloan_err_g^2)
