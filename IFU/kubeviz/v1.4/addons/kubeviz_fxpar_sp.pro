FUNCTION KUBEVIZ_FXPAR_SP, HDR, NAME, ABORT, COUNT=MATCHES, COMMENT=COMMENTS, $
                   START=START, PRECHECK=PRECHECK, POSTCHECK=POSTCHECK, $
                   NOCONTINUE = NOCONTINUE, $
                   DATATYPE=DATATYPE, HIERARCH=HIERARCH, HSIZE=HSIZE, K3D=K3D
;+
; NAME: 
;        FXPAR_SP()
; PURPOSE: 
...
;
;       current Version, KMOS3D, Nov 2014
;         added keyword /HIERARCH to deal with 'ESO HIERARCH' keywords
;         added keyword /K3D to deal with longer keywords of the form
;         K3D_+11 characters
;-
;------------------------------------------------------------------------------

IF KEYWORD_SET(K3D) then KEYWORDLENGTH = 11 else KEYWORDLENGTH=8
...
;
  ENDIF ELSE BEGIN
     WHILE STRLEN(NAM) LT KEYWORDLENGTH DO NAM = NAM + ' '
     VECTOR = 0
  ENDELSE
...
;
  ...
  IF NOT VECTOR AND START GE 0 THEN BEGIN
     ...
     KEYWORD = STRMID(HDR[MN:MX], 0, 11)
  ENDIF ELSE BEGIN
     RESTART:
     START   = -1L
     KEYWORD = STRMID( HDR, 0, KEYWORDLENGTH)
     if keyword_set(HIERARCH) then KEYWORD = STRMID( HDR, 13, 16) 
     if keyword_set(K3D) then KEYWORD = STRMID( HDR, 4, KEYWORDLENGTH)
  ENDELSE
...
;
  ENDIF ELSE BEGIN
     ...
     NFOUND = WHERE(STREGEX(KEYWORD, NAM, /BOO) EQ 1, MATCHES)
...
;
  ...
  IF MATCHES GT 0 THEN BEGIN
     ...
     IF KEYWORD_SET(HIERARCH) THEN BEGIN
        SVALUE = STRTRIM(STRMID(LINE,13+STRLEN(NAM)+2,71),2)
        EQPOS = STRPOS(SVALUE, "= '")
        IF EQPOS ge 0 THEN SVALUE = STRMID(SVALUE,EQPOS+2,70) 
     ENDIF
     IF KEYWORD_SET(K3D) THEN SVALUE = STRTRIM( STRMID(LINE,4+STRLEN(NAM)+2,71),2)
     IF KEYWORD_SET(HSIZE)   THEN SVALUE = STRTRIM( STRMID(LINE,HSIZE+1+STRLEN(NAM)+2,71),2)

     if keyword_set(K3D) then begin
        check1 = 'HISTORY    '
        check2 = 'COMMENT    '
        check3 = '           '
     endif else begin
        ...
        check3 = '        '
     endelse
     IF (NAM EQ check1) OR (NAM EQ check2) OR (NAM EQ check3) THEN BEGIN
        VALUE = STRTRIM( STRMID(LINE,KEYWORDLENGTH,72),2)
