...
readcol, fileinfo, ZP, errZP, AM, errAM, CLR, errCLR,skipline=1, numline=1
...
;weirdness: readcol deals with array
ZP=ZP[0] 
errZP=errZP[0]
AM=AM[0]
errAM=errAM[0]
...
;set to zero proper values
zero=where(MAG eq 0.,nzr)
if(nzr gt 0) then begin
MAG_calib[zero]=0.
MAG_error_cal[zero]=0.
SN[zero]=0.
Flux[zero]=0.
FluxErr[zero]=0.
endif
