...
;           x = ppxf_cap_range(-1d,1d,n_elements(galaxy))
...
;   V4.66 -- Uses PPXF_CAP_RANGE to avoid potential naming conflicts. 
...
;   V4.72 -- Replaced REBIN with INTERPOLATE with /OVERSAMPLE keyword. This is to 
...
;   V4.73 -- Allow for an input covariance matrix instead of an error spectrum.
;           MC, Oxford, 7 May 2014
;   V4.74 -- Fixed program stop with REDDENING keyword, introduced in V4.7.
...
n = 2*dx*factor + 1
x = ppxf_cap_range(dx,-dx,n)   ; Evaluate the Gaussian using steps of 1/factor pixel
...
;
x = ppxf_cap_range(-1d,1d,npix) ; X needs to be within [-1,1] for Legendre Polynomials
...
if factor gt 1 then pix = ppxf_cap_range(0d,s[1]-1d,s[1]*factor) ; Oversampled pixels range
...
repeat begin
    if regul gt 0 then begin
        aa = a[[goodPixels, ppxf_cap_range(npix*nspec,ncols-1)],*]
...
if keyword_set(plot) then begin
    ...
    for j=0,m-1 do begin
        x = ppxf_cap_range(goodPixels[w[j]],goodPixels[w[j]+1])
