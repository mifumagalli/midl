...
; long   --> return a long precision array
;-


function extractbox, fits, xpos, ypos, radius, double=double, long=long
...
;;allocate box
if keyword_set(double) then box=dblarr(2*rad+1,2*rad+1) 
if keyword_set(long) then box=lonarr(2*rad+1,2*rad+1) 
if ~keyword_set(long) and ~keyword_set(double)then box=fltarr(2*rad+1,2*rad+1)
