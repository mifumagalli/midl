...
function m_calclstar, z, H0=h0, AMIN=AMIN, SILENT=silent, $
...
common cosmolgy_cmmn, cosm_dm, cosm_K, cosm_h, cosm_Ob, cosm_L, cosm_r
  ...
  if (N_params() LT 1) then begin 
    print,'Syntax - ' + $
             'lstar = m_calclstar(z, H0=, AMIN=, /SILENT, '+$
	     	'LSTR0=, OM=, OV=) [v1.1]'
    return, -1
  endif 

;  if not keyword_set( LSTR0 ) then LSTR0 = -20.4  ;; Old value
  if not keyword_set( LSTR0 ) then LSTR0 = -21.06

  ;; Initialize the common block
  m_cosm_common, H0=h0, Omegavac=OV, OmegaDM=OM, SILENT=silent, _EXTRA=extra
  if not keyword_set(silent) then $
    print, 'm_calclstar: Assuming L_0 = ', strtrim(LSTR0,2), $
  ...
  if not keyword_set(silent) then begin
	print, 'm_calclstar:  Mpc per arcmin = ', amin
	print, 'm_calclstar:  Lstar = ', lstar
  endif	
