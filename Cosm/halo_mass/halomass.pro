...
function halomass, massmin, massmax, z, _EXTRA=extra , ndens=ndens, $
...
;;default
  ...
  if ~keyword_set(savehm) then begin
     if(z lt 10) then savehm='halomass_z'+string(z,format="(F5.3)")+rstring(Get_Tags(extra))+'.dat'
     if(z ge 10) then savehm='halomass_z'+string(z,format="(F5.2)")+rstring(Get_Tags(extra))+'.dat'
  endif
...
;;call the c++ program passing the cosmology
  spawn, [getenv("MIDL")+"/Cosm/halo_mass/psn",string(massmin),string(massmax),string(z),string(cosm_dm),string(littleh),string(sigma_8),string(prec)], output, /noshell
...
;;halo mass function
  if keyword_set(ndens) then begin
     ...
     for i=1, nbin_mass-1 do begin
        ...
        ;;compute function
        spawn, [getenv("MIDL")+"/Cosm/halo_mass/psn",string(minlimit),string(massmax),string(z),string(cosm_dm),string(littleh),string(sigma_8),string(prec)], output, /noshell
