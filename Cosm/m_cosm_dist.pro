...
function m_cosm_intdist, z
common cosmolgy_cmmn, cosm_dm, cosm_K, cosm_h, cosm_Ob, cosm_L, cosm_r, sigma_8

  distintg = 1./ m_cosm_hubble(z)
  return, distintg
end
  

function m_cosm_intdist2, z
common cosmolgy_cmmn, cosm_dm, cosm_K, cosm_h, cosm_Ob, cosm_L, cosm_r, sigma_8

  distintg = 1./ m_cosm_hubble(z) / (1+z)
  return, distintg
end
  


function m_cosm_dist, z, H0=h0, INIT=init, LUM=lum, _EXTRA=extra, SILENT=silent, PHYSICAL=physical

  common cosmolgy_cmmn

  if (N_params() LT 1) then begin 
    print,'Syntax - ' + $
             'dist = m_cosm_dist(z, H0=, /INIT, /LUM) [Mpc; v1.1]'
    return, -1
  endif 
...
;  resolve_routine, 'm_cosm_hubble', /no_recompile, /is_function
  if not keyword_set(PHYSICAL) then $
          dist = qromb('m_cosm_intdist', 0., z, /double) else $
                  dist = qromb('m_cosm_intdist2', 0., z, /double) 
  
  c = x_constants()

  if keyword_set(LUM) then fact = (1+z) else fact = 1.

  if not keyword_set(SILENT) then print, 'm_cosm_dist: Units are in Mpc'
