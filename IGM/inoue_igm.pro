...
; 
;  filter      the filter used for convolution  
;  rwave,tigm  output IGM absorption in restframe wave 
;  limit       if set, set flux ablue to 912*(1+zemi) to zero 
;  IGMmag      correction in magnitude  [mag_true=mag_obs-IGMmag]
;  beta        slope used to weight the correction 
;  check       do some check plots 
;-



PRO inoue_igm, zemi, filter=filter, tigm=tigm, rwave=rwave, IGMmag=IGMmag, $
               beta=beta, check=check, limit=limit
  ...
  ;;inser limit if told to do so 
  if keyword_set(limit) then tigm[where(rwave lt 912.)]=0.
  ...
  IGMmag=-2.5*alog10(total(wrest^(beta+1)*tigmc*transf*deltal)/total(wrest^(beta+1)*transf*deltal))
