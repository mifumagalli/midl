...
for f=0, nfilt-1 do begin
    ...
    for o=0L, nobj-1 do begin
        ...
        if keyword_set(bpz) then begin
            ...
            ;;fix extremely small mag
            if(flux gt 30 or errflux gt 30) then begin
                flux=-99.0
                errflux=0.0
            endif

            ;;99 is already set for non detected
        ...
        ;;print stuff
        if keyword_set(eazy) then begin
            if(f eq 0) then line[o]=string(obj[o].number[0],zspec[o],flux,errflux,$
                                           format='(I8," ",F," ",E," ",E," ")')
        ...
        if keyword_set(bpz) then begin
            if(f eq 0) then line[o]=string(obj[o].number[0],zspec[o],flux,errflux,$
                                           format='(I8," ",F20," ",F20," ",F20," ")')
            if(f gt 0) then line[o]=line[o]+string(flux,errflux,format='(" ",F20," ",F20," ")') 
            if(f eq nfilt-1) then printf, lun, line[o]   
        endif
