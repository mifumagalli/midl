...
function m_cosm_b_inttime, v
common cosmolgy_cmmn, cosm_dm, cosm_k, cosm_h, cosm_Ob, cosm_L, cosm_r, sigma_8

  t = exp(v) - 1
  distintg = 1./ m_cosm_hubble(t)
  return, distintg
end

function m_cosm_time, z, H0=h0, INIT=init, _EXTRA=extra, START=start

common cosmolgy_cmmn, cosm_dm, cosm_k, cosm_h, cosm_Ob, cosm_L, cosm_r, sigma_8

  if (N_params() LT 1) then begin 
    print,'Syntax - ' + $
             'age = m_cosm_time(z, H0=, /INIT, /W06MAP, /START) [v1.1]'
    return, -1
  endif 
  ...
  if not keyword_set(START) then begin
      for jj=0L,nz-1 do time[jj] = qromb('m_cosm_inttime', 0., z[jj]>0., /double) 
  endif else begin
      time = qromo('m_cosm_b_inttime', alog(z+1), /midexp, /double)
  endelse
