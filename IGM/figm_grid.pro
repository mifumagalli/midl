...
;
;
;filter_tra   file of filter transmission curve (lambda in AA and
;transmission) - or index of file sotored in master.FILTER.res
...
;             for each z step
;INDEX        set the filter_tra to index in master.FILTER.res
;
;
...
PRO figm_grid, filter_tra, z_out, IGM_corr, TYPE=type, SAVENAME=savename, DISPLAY=display, INDEX=INDEX

;set default
if ~keyword_set(type) then type='P10full' 

;open filter transmission curve
if keyword_set(index) then getfilter, filter_tra, lambda=Lfilter, trans=Tfilter else $
