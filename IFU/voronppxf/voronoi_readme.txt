This archive contains the IDL program voronoi_2d_binning.pro
...
voronoi_2d_binning_example.pro.
...
voronoi_2d_binning_example.pro, to specify the desired target S/N of
your final bins;

3) Run the program by typing "voronoi_2d_binning_example" at the IDL
prompt and wait for the final plot to appear. The output is saved in
the text file voronoi_2d_binning_output.txt. The column BIN_NUM is all
is needed to actually bin the data.

4) Read the documentation at the beginning of the file
voronoi_2d_binning.pro to fully understand the meaning of the various
...
of connected pixels *separately* (LABEL_REGION can be used in IDL to
find connected regions). Alternatively one may optimally weight the
pixels before binning. See Sec.2.1 of Cappellari & Copin (2003) for
details.
...
Last software update 19 March 2014
