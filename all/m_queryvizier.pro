function M_Queryvizier, catalog, target, dis, VERBOSE=verbose, CANADA = canada, $
               CONSTRAINT = constraint, ALLCOLUMNS=allcolumns, SILENT=silent, found=found
;+
; NAME: 
;   QUERYVIZIER
;
...
;Check if any Warnings or fatal errors in the VIZIER output
 ...
 case strmid(fmt[i],0,1) of 
  ...
  i0 = max(lcol) + 4
  if(n_elements(t) ge i0) then iend = where( t[i0:*] EQ '', Nend) else Nend=0
  if Nend EQ  0  then iend = N_elements(t) else iend = iend[0] + i0
  nstar = iend - i0 
  ;flag -1 in no star found
