...
; CALLING SEQUENCE:
;   
;   mcombine, img, comb, [header], FCOMB=, GAIN=, RN=, MASKS=, SCALE=,
;     STATSEC=, MMEM=, SIGHI=, SIGLO=, MXITER=, OUTNM=, /SILENT, 
;     WEIGHT=, IMGINDX=, OUTMASK
;
; INPUTS:
...
;   outmask  -- Array containing the final mask applied  
;
; OPTIONAL OUTPUTS:
...
pro mcombine, img, cmbimg, header, FCOMB=fcomb, SCALE=scale, MMEM=mmem, $
              MASKS=masks, $
...
;
  if  N_params() LT 1  then begin 
      print,'Syntax - ' + $
        'mcombine, img, [cmbimg, header], FCOMB=, MMEM=, MASKS=, SIGLO=' 
...
; Status
	...
	' of ', strtrim(n2_0-1,2)
      ...
      case fcomb of 
          ...
          4 : $                 ; Average with Mask
	     comb[*, srow:lrow] =djs_avsigclip(imgarr, $
                                             MAXITER=mxiter, $
                                             INMASK=mskarr,$
					     OUTMASK=outmask)
      ...
      endcase
      ;if sigmaclipped set, compress in a single mask
      IF(FCOMB EQ 7) THEN BEGIN 
      compressmask=outmask[*,*,0]-outmask[*,*,0]
      FOR idf=0, nimg-1 DO  compressmask=(compressmask+outmask[*,*,idf])
      nozero=where(compressmask GT 0.,nnn)
      outmask=compressmask-compressmask
      IF(nnn GT 0) THEN outmask[nozero]=1.
      ENDIF
      
      
      ;if sigmaclipped not set, compress in a single mask input mask
      IF(FCOMB EQ 4) THEN BEGIN
      compressmask=mskarr[*,*,0]-mskarr[*,*,0]
      FOR idf=0, nimg-1 DO  compressmask=(compressmask+mskarr[*,*,idf])
      nozero=where(compressmask GT 0.,nnn)
      outmask=compressmask-compressmask
      IF(nnn GT 0) THEN outmask[nozero]=1.
      ENDIF
